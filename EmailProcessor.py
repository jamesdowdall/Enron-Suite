...
__CSVHEADERS__ = ["Email_ID","Parent_Folder","User","Date","Subject","Email_From","Email_To","Body","Cc","Bcc","Number_of_Recipients","Length_of_Email_Body","Message_ID","Importance_Statement","Thread"]#,"FilePath"]
...
__WEKAHEADERS__ = ["Email_ID","Importance_Statement","Thread","Number_of_Recipients","Length_of_Email_Body","Number_of_Tos","Number_of_Ccs","Number_of_Bccs"] + __LIWCHEADERS__
__OUTPUT__ = "WEKA"
...
def normaliseResults(results, lower, upper):
	...
	for result in results:
		...
		#print result
		while j < len(result):
			if upperminuslower[j-3] == 0.0: #If denom is 0
				result[j] = 0.0
			else: 
				result[j] = (float(result[j]) - lower[j-3])/upperminuslower[j-3]
			j += 1 
...
if __name__ == "__main__":
	...
	for email in emailSelection.keys():
		...
		try:
			...
			data['Importance_Rating'] = None

		processedData = processData(data)
		preparedData = prepareDataForWrite(emailID,user,parentFolder,processedData)

		#Cutting down to only useful values
		#EmailID, Importance Statement, Thread, Number or Recipients, Email Length, Number of Tos, Number of Ccs, Number of Bccs
		preparedData = [preparedData[0]] + [preparedData[14]] + [preparedData[18]] + [preparedData[10]] + [preparedData[11]] + [preparedData[15]] + [preparedData[16]] + [preparedData[17]] 
		...
		if (__NORMALISED__):#Ensure min & max are correct length & full of unlikely values
			while (len(minimum) < (len(data)-3)):
				minimum.append(100.0)
				maximum.append(0.0)

			j = 3 #Ignore emailID & Importance Statement & Thread
			while j<len(data):
				if float(str(data[j])) < minimum[j-3]:
					minimum[j-3] = float(str(data[j]))
				elif float(str(data[j])) > maximum[j-3]:
					maximum[j-3] = float(str(data[j]))
				j += 1
